    ...
    + table([th("Area", "範疇"), th("Reviewable: a specified decision", "可覆核：屬指明決定"), th("Not on the list", "不在清單之內")], [
        tr(rh("Your licence", "你的牌照"),
           ...
           td("The licence ending by itself on death, dissolution or the start of winding up, and your own decision to cease business. Neither is a decision of the Commissioner.",
              "牌照因去世、解散或開始清盤而自動失效，以及你自行決定停業。兩者都不是關長的決定。", "s.42 · s.41")),
        ...
        tr(rh("Discipline", "紀律行動"),
           td("Any power under section 21 (Part 4), for breach of a Schedule 2 specified provision, and any power under section 43 (Part 5), for breach of a licence condition, a s.51 regulation or a licensee duty in ss.35–41: public reprimand, remedial order, pecuniary penalty.",
        ...
        tr(rh("Supervision and prosecution", "監管及檢控"),
           ...
           td("A routine inspection or an investigation requirement, a magistrate's warrant, a decision to prosecute, and a notice to keep records for longer than five years.",
              "例行視察或調查要求、裁判官手令、檢控決定，以及要求把紀錄保存超過五年的通知。", "Part 3 · s.79 · s.20(4) Sch. 2", post=flag())),
    ], minw=780)
    + traps(
...
C_ = sec('clock', ["s.59", "s.75", ("with s.30–34", "另及第30至34條")],
    ...
    + traps(
        trap(("Not every decision waits for the 21 days", "並非每項決定都等待21日"), None, "s.30(7) · s.34(6) · s.75",
             vs=[(("Waits", "等待"), ("Penalties, reprimands, remedial orders, refusals: the ordinary rules apply, so an application for review keeps them from taking effect until it is decided or withdrawn, unless the notice set another time in the public interest.", "罰款、譴責、糾正命令、拒絕：適用一般規則，故申請覆核後，決定要待覆核有結果或申請撤回時才生效，除非通知為公眾利益另定時間。")),
...
D_ = sec('powers', ["s.61–s.68", ("powers and offences", "權力與罪行")],
    ...
    + table([th("Power", "權力"), th("What it lets the Tribunal do", "審裁處可以做甚麼")], [
        ...
        tr(rh("Enforcement", "強制執行", "s.67–s.68"), td("Its orders are written and signed by the chairperson, presumed genuine, and can be registered in the Court of First Instance, after which they count as that court's orders.", "其命令以書面記錄並由主席簽署，推定為妥為作出，並可在原訟法庭登記，登記後即視為該法庭的命令。")),
    ], minw=700)
    ...
    + traps(
        trap(("Self-incrimination: automatic here, claim-first in an investigation", "入罪保障：此處自動適用，調查時須先聲稱"), None, "s.61(4) · s.62 · s.15",
