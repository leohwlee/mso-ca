...
CMP = table(
    ...
    ''.join([
        ...
        + d("An <b>investigator</b>. The Commissioner <b>directs</b> one of his own Customs and Excise officers, or with the Financial Secretary's consent <b>appoints</b> someone from outside.", "<b>調查員</b>。由關長<b>指示</b>其海關人員擔任，或在財政司司長同意下<b>委任</b>外部人士。", "s.8 · s.11(1)–(2)") + '</tr>',
        ...
        + d("They may ask a third party who holds your records, but <b>only as a last resort</b>, after forming reasonable cause to believe they cannot get it from you.", "可向持有你紀錄的第三方索取，但<b>只可作為最後手段</b>，且須先有合理因由相信無法從你處取得。", "s.9(6)–(7)")
        ...
        + d("If the officer was authorised by a regulator that is <b>not your own</b>, nothing about a customer need be disclosed unless that regulator <b>certifies in writing</b> that it is necessary.", "如該人員由<b>並非你的</b>監管當局授權，除非該當局<b>藉書面證明</b>屬必要，否則毋須披露任何客戶資料。", "s.9A")
...
WHEN = table(
    ...
    ''.join([
        ...
        + d("That person is an <b>information holder</b>. On a routine inspection this is allowed <b>only</b> where the officer has reasonable cause to believe the material cannot be obtained from you. During an investigation there is no such condition.", "該人屬<b>資料持有人</b>。在例行視察中，<b>只有</b>當人員有合理因由相信無法從你處取得有關資料時方可如此；在調查中則沒有此條件。", "s.9(6)–(7) · s.12(1)") + '</tr>',
        '<tr>' + rh("The officer was sent by a regulator other than the Commissioner and asks about a customer", "該人員由關長以外的監管當局派出，並查詢客戶資料")
        + d("You need not disclose anything about a customer's affairs <b>unless that other regulator certifies in writing</b> that the disclosure is necessary. In an investigation they must also reasonably believe that customer has relevant information.", "除非<b>該其他監管當局藉書面證明</b>披露屬必要，否則你毋須披露任何客戶事務資料。在調查中，他們另須有合理因由相信該客戶掌握相關資料。", "s.9A · s.12A") + '</tr>',
...
STD = table(
    ...
    ''.join([
        ...
        + d("An investigator showing you the direction or appointment. Note how much earlier this is than the inspector's duty above.", "調查員須於此時向你出示指示或委任文本。留意這比上述視察人員的責任要早得多。", "s.11(5)") + '</tr>',
...
PEN = table(
    ...
    ''.join([
        ...
        '<tr>' + d("Produced a record or gave an answer that was <b>false or misleading in a material particular</b>, knowing it or not caring", "交出的紀錄或給予的回答<b>在要項上屬虛假或具誤導性</b>，且明知或罔顧實情")
...
FSEC = sec('traps', [("easy marks", "易失分處"), ("if you read", "只要細讀"), ("carefully", "措辭")],
    ...
    U.traps(
        ...
        U.trap(("Only an investigator can make you sit down", "只有調查員能要求你坐下"),
               ("An inspector can require you to hand over a record at a stated time and place, and to answer questions about it. The power to require a person to <b>attend</b> before the officer exists only in the investigation section.", "視察人員可要求你在指明時間及地點交出紀錄，並回答有關問題。要求某人<b>會晤</b>該人員的權力，只見於調查條文。"), "s.12(2)(b)"),
        U.trap(("The last-resort rule applies to one track only", "最後手段規則只適用於一條途徑"),
               ("Approaching a third party who holds your records is a <b>last resort during an inspection</b>: the officer must first have reasonable cause to believe you cannot supply it. During an investigation that third party can be approached directly.", "在<b>視察</b>中向持有你紀錄的第三方索取屬<b>最後手段</b>：人員須先有合理因由相信你無法提供。在調查中則可直接向該第三方索取。"), "s.9(6)–(7) · s.12(1)"),
        U.trap(("Two different moments for showing authority", "出示授權的兩個不同時點"), None, "s.9(14) · s.11(5)",
               vs=[(("An inspector", "視察人員"), ("Shows the written authorisation as soon as reasonably practicable while using the power.", "在行使權力期間盡快出示書面授權。")),
        ...
        U.trap(("You must answer, but the answer can be sealed off", "你必須回答，但回答可獲封存"),
               ("Self-incrimination is <b>no excuse</b> for refusing to comply with an investigator's requirement. The protection is narrower than people expect: only if you claim it <b>before answering</b> do the requirement, the question and the answer become inadmissible against you in criminal proceedings, and even then not if you are charged over that answer itself under s.13 or Part V of the Crimes Ordinance.", "可能導致入罪<b>不能作為</b>拒絕遵從調查員要求的理由。該保障比一般人以為的窄：只有在<b>回答之前</b>作出聲稱，該要求、有關問題及回答才不得在刑事法律程序中接納為針對你的證據；即使如此，若你就該回答本身被控犯第13條或《刑事罪行條例》（第200章）第V部所訂罪行，該保障亦不適用。"), "s.13(11) · s.15"),
        U.trap(("That protection does not cover a routine visit", "該保障不涵蓋例行到訪"),
               ("The self-incrimination shield is written around requirements imposed <b>by an investigator</b>. Nothing you say to an authorized person on a routine inspection is protected by it, and it also falls away if you are charged over the answer itself under s.13 (for example, for a false answer) or under Part V of the Crimes Ordinance.", "入罪保障是圍繞<b>調查員</b>施加的要求而訂。你在例行視察中向獲授權人所說的話，一律不受保障；若你就該回答本身被控犯第13條所訂罪行（例如給予虛假回答），或《刑事罪行條例》（第200章）第V部所訂罪行，該保障亦不適用。"), "s.15(2)–(3)"),
        ...
        U.trap(("Breaking a licence condition is breaking the rules", "違反牌照條件即屬違規"),
               ("The standard an inspector checks you against is not only the Ordinance. It also covers any notice or requirement given under it, <b>the conditions on your licence</b>, any conditions on a registration, and any other condition imposed under the Ordinance.", "視察人員用以衡量你的標準，不限於本條例本身，還包括據此給予的任何通知或要求、<b>你牌照上的條件</b>、任何註冊的條件，以及據本條例施加的任何其他條件。"), "s.8"),
        U.trap(("The power to walk in stops at the registered premises", "進入權力只及於已登記的處所"),
